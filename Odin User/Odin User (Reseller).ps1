...
Try 
{
    ...
    $BAXML = $BAXML -replace "{account}", "1000003"
    $BAXML = $BAXML -replace "{username}", "cob_{user name}"
    ...
    $cmdresults = Invoke-WebRequest -Uri http://{BAIP}:5224 -Credential $odincreds -Method POST -Body $BAXML
...
#Create Log Entry
Add-Content {log path} "$date`r`nOdin User Addition (Reseller)`r`nLog: $ErrorTrace"